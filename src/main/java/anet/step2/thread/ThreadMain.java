...
순서는 운영체제에 의해서 결정됨
같은 PC라도 요청할 때마다 순서가 달라질 수도 있다.

CPU코어가 2개일 때
: 2개이므로 물리적으로 정말 동시에 실행될 수도 있고
: 하나의 CPU코어에 시간을 나누어서 실행될 수도 있는 것임
...
List(인터페이스) -> ArrayList(단일스레드만 안전), Vector(멀티스레드 안전함 - 나는 ArrayList보다 읽고쓰기가 느려)
