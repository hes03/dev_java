...
public class AppleDao {
    ...
    public ZipCodeVO[] getAddress(String zdo, String dong){
        ...
        try {
            ...
            //한달에 한번씩 계정에 대한 비번을 변경하더라도 한 곳만 고치면
            //모든 소스에 적용된다.
            ...
            //executeQuery는 리턴타입이 ResultSet
            //입력, 수정, 삭제일때는 executeUpdate를 사용한다.
            //executeUpdate는 리턴타입이 int
            ...
            v.copyInto(zvos);
            //문제제기: 만일 두번 연속해서 조회할 경우 앞에 조회결과가 남아 있다.
            //기존에 조회된 결과는 삭제하고 새로 출력한다.
            //만일 참이면 조회결과가 있다. 이고 만일 거짓이면 조회결과가 없다.
            if (v.size() > 0) {
    ...
    public String[] getZdoList(){
        ...
        try {
            ...
            System.out.println(zdos.length); //0
            v.copyInto(zdos);
            System.out.println(zdos.length); //17
        }catch(SQLException se){
