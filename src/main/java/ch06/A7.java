...
/*
원시형 타입 - 호출하면 값이 출력됨
byte와 short 사용하지 않음. 왜냐면 어차피 JVM이 처리할 때 int로 자동형전환됨
char < short< byte < int < long < float < double
boolean
참조형 타입 - 호출하면 16진수 주소번지 출력됨 - 간접참조방식 - 재네릭

정적변수(static <-> nonstatic) - 하나를 공유함
인스턴스화가 필요없음
주의 : static 영역에서 non static 타입을 사용불가함
 */
public class A7 {
