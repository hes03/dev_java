...
//기지국에서 1초에 한번씩 보내주는 시간정보를 청취하기
//지속적으로 1초에 한번씩 실행문을 처리하기 위해서 Thread 상속받음
public class TimeClient extends Thread{
    /*
    서버소켓과 통신은 TimeClient에서 하고 서버소켓에서 써준 현재 시간정보도
    TimeClient를 통해서 읽어올 수 있다.
    즉, 현재 시간정보는 TimeClient에 있는데 시간정보에 대한 출력은
    TimeDisplay에서 해야 한다.
    TimeDisplay에서 TimeClient를 인스턴스화 해야한다.
    ...
    public void run(){
        ...
        String time = null;
        try {
            socket = new Socket("192.168.0.22", 3000);
            ...
            while(true){
                while((time = in.readLine())!=null){
                    ...
                    jlb_time.setText(time); //NullPointerException 발생
                }
                try{
                    sleep(1000);
                }catch (InterruptedException e){
                    e.getMessage(); //스레드 관련 에러 발생 시 출력하기
                }//end of inner while
            }//end of outer while
        } catch (Exception e) {
    ...
    public static void main(String[] args) {
        ...
        //run() 직접 호출하지 않고 start()호출하면 JVM이 대신 호출 해줌
        //timeClient.start();
    }//end of main
}//end of TimeClient
